...
<<<<<<< HEAD
V skutočnosti si ale Python sám pre seba eviduje návratovú hodnotu, ale pokial to programátor nepožaduje získať tým že definuje príslušný parameter, tak si to Python necháva iba pre seba. Ak programátor ale potrebuje získať z funkcie navratovú hodnotu použije za týmto účelom v tele funkcie príkaz **return**. Takejto funkcii sa hovorí že je to **funkcia s návratovou hodnotou** V ostatných prípadoch však funkcia vracia **None** čo je pythonovský ekvivalent hodnoty nič, žiadna hodnota, neznáma hodnota, neidentifikovatelná hodnota a pod. **Nie je to však nula ako číslo.**
=======
